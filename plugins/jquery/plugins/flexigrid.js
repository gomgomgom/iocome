...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			...
			doAfterPopulate : null,
			my_name : '',
			onAfterPopulateView: false
		}, p);
...
*/
				...
				(
					function ()
					{
						...
						if($.browser.msie==false && $.browser.mozilla==false){
							cdpos = cdpos + cdleft + cdpad;
						}
			...
			addData: function (data) { //parse data
				...
				if (p.onSuccess) eval(p.onSuccess);
			...
			populate: function () { //get latest data
				...
				$.ajax({
					...
					success: function(data){
						...
						//execute after populate	
						if(p.doAfterPopulate!=null)
						{
							eval(p.doAfterPopulate);
						}	
		...
		doSearch: function () {
			...
			{
				if($('input[name=q]',g.tDiv).val()=="Quick Search")
				{
					p.query = "";	
				}
				else
	...
	g.sDiv = document.createElement('div'); //search
	...
	// added callback after populate view
	if(typeof p.onAfterPopulateView == 'function')
	{
		p.onAfterPopulateView();
	}
...
$.fn.flexReload = function(p) { // function to reload grid
	return this.each( function() {
		if (this.grid&&this.p.url)
		{
			if(p)
			{
				this.p.url = p.url || this.p.url;
				this.p.params = p.params || this.p.params;
				this.p.qtype = p.qtype != undefined ? p.qtype : this.p.qtype;
				this.p.query = p.query != undefined ? p.query : this.p.query;
			}
			this.grid.populate();
		}
	});
